node {
    ...
    stage ('Xray artifactory scan') {
      ...
      // try{
        ...
        sh """curl --user $creds --header 'Content-Type: application/json' --request POST --data '{"builds":[{"name":"${buildName}"}]}' https://jfartifactory.resolve.local:8081/xray/api/v1/violations/ignored |jq '.data[] | select(.impacted_artifact.version == "${BUILD_NUMBER}")' >> IgnoredViolation-${BUILD_NUMBER}.log"""        
