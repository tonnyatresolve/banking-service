node {
    ...
    stage ('Xray artifactory scan') {
      ...
      // try{
        ...
        sh "curl --user $creds https://jfartifactory.resolve.local:8081/xray/api/v1/violations/ignored --header 'Content-Type: application/json' --request POST --data '{"builds": [{"name": \"${buildName}\"}]}'|jq >> IgnoredViolation-${BUILD_NUMBER}.log"
